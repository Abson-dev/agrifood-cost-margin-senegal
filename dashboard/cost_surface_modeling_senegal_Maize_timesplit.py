...
Maize-only transaction cost analysis for Senegal using commodity_id 56
All outputs saved in maize_analysis_outputs folder
...
COST_SURFACE_RF_PLOT_FILE = os.path.join(OUTPUT_DIR, 'cost_surface_rf_maize.png')
COST_SURFACE_XGB_PLOT_FILE = os.path.join(OUTPUT_DIR, 'cost_surface_xgb_maize.png')
SCATTER_RF_PLOT_FILE = os.path.join(OUTPUT_DIR, 'scatter_cost_vs_distance_rf_maize.png')
SCATTER_XGB_PLOT_FILE = os.path.join(OUTPUT_DIR, 'scatter_cost_vs_distance_xgb_maize.png')
MODEL_COMPARISON_PLOT_FILE = os.path.join(OUTPUT_DIR, 'model_comparison_maize.png')
MODEL_COMPARISON_CSV = os.path.join(OUTPUT_DIR, 'model_comparison_maize.csv')
FEATURE_IMPORTANCE_EXCEL = os.path.join(OUTPUT_DIR, 'feature_importance_maize.xlsx')
FEATURE_IMPORTANCE_PLOT_FILE = os.path.join(OUTPUT_DIR, 'feature_importance_maize.png')
SHAPEFILE_PATH = 'senegal_shapefile/sen_admbnda_adm1_anat_20240520.shp'
RF_MODEL_FILE = os.path.join(OUTPUT_DIR, 'rf_model_maize.pkl')
XGB_MODEL_FILE = os.path.join(OUTPUT_DIR, 'xgb_model_maize.pkl')
...
MAIZE_SPOILAGE_RATE = 0.1  # From PERISHABILITY_MAPPING for commodity_id 56
...
# Filter for Maize using commodity_id 56
df = df[df['commodity_id'] == 56]
if df.empty:
    print("Error: No data for Maize (commodity_id 56) found in the dataset.")
    sys.exit(1)
print(f"Filtered data for Maize (commodity_id 56), DataFrame shape: {df.shape}")
...
# Assign constant spoilage rate for Maize
df['spoilage_rate'] = MAIZE_SPOILAGE_RATE
print("Spoilage rate for Maize set to 0.1.")
...
# Interaction terms
df['month_spoilage_interaction'] = df['month_sin'] * MAIZE_SPOILAGE_RATE  # spoilage_rate is constant
...
df['spoilage_cost'] = MAIZE_SPOILAGE_RATE * df['price_farmgate']
df['transaction_cost_xof_per_kg'] = df['transport_cost'] + df['spoilage_cost']
print("Transaction costs (XOF/kg) estimated for Maize.")
...
print("\nModel Comparison for Maize (Time Series Cross-Validation on Training Data):")
...
ax.set_title('Feature Importance for Maize Transaction Cost Models', fontsize=16, pad=15)
...
ax.set_title('Cross-Validation Metrics for Maize Transaction Cost Models', fontsize=16, pad=15)
ax.set_xticks(x)
...
ax.set_title('Maize Transaction Cost vs. Distance (Random Forest, Test Set)', fontsize=16, pad=15)
ax.legend(title='Transaction Cost', frameon=True, fontsize=10)
ax.grid(True, which='major', linestyle='--', alpha=0.5)
plt.tight_layout()
plt.savefig(SCATTER_RF_PLOT_FILE, bbox_inches='tight')
plt.close()
print(f"Scatter plot (Random Forest, Maize, Test Set) saved to '{SCATTER_RF_PLOT_FILE}'")
...
ax.set_xlabel('Distance (km)', fontsize=14)
ax.set_ylabel('Transaction Cost (XOF/kg)', fontsize=14)
ax.set_title('Maize Transaction Cost vs. Distance (XGBoost, Test Set)', fontsize=16, pad=15)
ax.legend(title='Transaction Cost', frameon=True, fontsize=10)
ax.grid(True, which='major', linestyle='--', alpha=0.5)
plt.tight_layout()
plt.savefig(SCATTER_XGB_PLOT_FILE, bbox_inches='tight')
plt.close()
print(f"Scatter plot (XGBoost, Maize, Test Set) saved to '{SCATTER_XGB_PLOT_FILE}'")
...
# --- End of Script ---
print(f"\nMaize analysis complete. All outputs saved in '{OUTPUT_DIR}' folder.")