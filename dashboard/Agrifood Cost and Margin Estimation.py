...
@st.cache_data
def load_commodity_data(region_file='commodity_prices_merged.xlsx', market_file='wfp_food_prices_sen.xlsx'):
    try:
        # Load region-level data (if available)
        region_df = None
        if os.path.exists(region_file):
            region_df = pd.read_excel(region_file, engine='openpyxl')
            ...
            if missing_columns:
                st.error(f"Missing required columns in region file: {', '.join(missing_columns)}")
                region_df = None
            else:
                # Validate data types
                region_df['Year'] = pd.to_numeric(region_df['Year'], errors='coerce')
                region_df['Month'] = pd.to_numeric(region_df['Month'], errors='coerce')
                region_df['Régions - Latitude'] = pd.to_numeric(region_df['Régions - Latitude'], errors='coerce')
                region_df['Régions - Longitude'] = pd.to_numeric(region_df['Régions - Longitude'], errors='coerce')
                region_df['Price'] = pd.to_numeric(region_df['Price'], errors='coerce')
                region_df['Unit'] = region_df['Unit'].astype(str).fillna('Unknown')
        ...
        market_df = pd.read_excel(market_file, engine='openpyxl')
        required_columns = ['date', 'market', 'market_id', 'latitude', 'longitude', 'commodity', 'price', 'unit']
        missing_columns = [col for col in required_columns if col not in market_df.columns]
        if missing_columns:
            st.error(f"Missing required columns in market file: {', '.join(missing_columns)}")
            return None, None
        # Process date column
        ...
        # Rename columns to match region-level data
        market_df = market_df.rename(columns={
            'market': 'Régions Name',
            'market_id': 'Régions - RegionId',
            'latitude': 'Régions - Latitude',
            'longitude': 'Régions - Longitude',
            'commodity': 'Commodity',
            'price': 'Price',
            'unit': 'Unit'
        })
        ...
        market_df['Month'] = pd.to_numeric(market_df['Month'], errors='coerce')
        market_df['Régions - Latitude'] = pd.to_numeric(market_df['Régions - Latitude'], errors='coerce')
        market_df['Régions - Longitude'] = pd.to_numeric(market_df['Régions - Longitude'], errors='coerce')
        market_df['Price'] = pd.to_numeric(market_df['Price'], errors='coerce')
        market_df['Unit'] = market_df['Unit'].astype(str).fillna('Unknown')
        # Check for invalid data
        ...
        st.error(f"Market file '{market_file}' not found.")
        return None, None
...
def generate_map(region_df, market_df, year, month, map_style, travel_png_path, friction_png_path, image_bounds, markets, roads, selected_commodities):
    ...
    if region_df is not None:
        region_filtered = region_df[(region_df['Year'] == year) & (region_df['Month'] == month) & (region_df['Commodity'].isin(selected_commodities))]
        if region_filtered.empty:
            st.warning(f"No region-level commodity data found for Year {year}, Month {month}, and selected commodities.")
            region_grouped = None
        else:
            region_grouped = region_filtered.groupby(['Régions Name', 'Régions - RegionId', 'Régions - Latitude', 'Régions - Longitude']).agg({
                'Commodity': list,
                'Price': list,
                'Unit': list
            }).reset_index()
            region_grouped['commodity_count'] = region_grouped['Commodity'].apply(len)
            regions_mapped = region_grouped['Régions Name'].tolist()

            # Add region-level commodity markers
            for _, row in region_grouped.iterrows():
                ...
                folium.CircleMarker(
                    ...
                    tooltip=f"{row['Régions Name']}: {commodity_count} commodities (Region)",
                    fill=True,
                    fill_color=color,
                    color=color,
                    fill_opacity=0.7
                ).add_to(folium.FeatureGroup(name="Region Commodities").add_to(m))
    else:
        region_grouped = None

    # Process market-level data
    market_filtered = market_df[(market_df['Year'] == year) & (market_df['Month'] == month) & (market_df['Commodity'].isin(selected_commodities))]
    if market_filtered.empty:
        st.warning(f"No market-level commodity data found for Year {year}, Month {month}, and selected commodities.")
        market_grouped = None
    else:
        market_grouped = market_filtered.groupby(['Régions Name', 'Régions - RegionId', 'Régions - Latitude', 'Régions - Longitude']).agg({
            'Commodity': list,
            'Price': list,
            'Unit': list
        }).reset_index()
        market_grouped['commodity_count'] = market_grouped['Commodity'].apply(len)
        regions_mapped.extend(market_grouped['Régions Name'].tolist())

        # Add market-level commodity markers
        for _, row in market_grouped.iterrows():
            ...
            popup_content = f"""
            <div style='width: 250px'>
                <h4>{row['Régions Name']} (Market)</h4>
                <b>Market ID:</b> {row['Régions - RegionId']}<br>
                <b>Commodities ({commodity_count}):</b><br>{commodity_list}<br>
                <b>Coordinates:</b> {row['Régions - Latitude']:.4f}, {row['Régions - Longitude']:.4f}
            </div>
            """
            folium.CircleMarker(
                ...
                popup=folium.Popup(popup_content, max_width=300),
                tooltip=f"{row['Régions Name']}: {commodity_count} commodities (Market)",
                fill=True,
                fill_color='green',
                color='green',
                fill_opacity=0.7
    ...
    # Combine filtered data for table
    combined_filtered = pd.concat([region_filtered, market_filtered]) if region_filtered is not None and not region_filtered.empty else market_filtered
    ...
    return m, regions_mapped, combined_filtered

def main():
    st.title("Senegal Commodity and Geospatial Analysis Map")
    st.markdown("Explore commodity availability, prices, travel time to cities, and friction surfaces across Senegal at region and market levels.")
    ...
    # Load commodity data
    region_df, market_df = load_commodity_data()
    if market_df is None and region_df is None:
        return

    # Commodity filter
    commodities = []
    if region_df is not None:
        commodities.extend(region_df['Commodity'].dropna().unique())
    if market_df is not None:
        commodities.extend(market_df['Commodity'].dropna().unique())
    ...
    months = []
    if region_df is not None:
        years.extend(region_df['Year'].dropna().unique().astype(int))
        months.extend(region_df['Month'].dropna().unique().astype(int))
    if market_df is not None:
        years.extend(market_df['Year'].dropna().unique().astype(int))
        months.extend(market_df['Month'].dropna().unique().astype(int))
    ...
    with col1:
        selected_year = st.selectbox("Select Year", years, index=years.index(2000) if 2000 in years else 0)
    with col2:
    ...
    map_obj, regions_mapped, filtered_df = generate_map(
        region_df, market_df, selected_year, selected_month_num, map_style, 
    ...
    if map_obj:
        ...
        if regions_mapped:
            st.write(f"Regions/Markets mapped: {', '.join(set(regions_mapped))}")
        else:
            st.write("No commodity regions or markets to display for the selected year, month, and commodities.")
